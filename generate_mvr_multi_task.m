function [S,T,X_test_target,y_test,M,Ct,X_test_source] = generate_mvr_multi_task(ns,nst,p,m,k,beta,rt)
...
%                   Ct(:,:,m*(s-1)+jf)=1*toeplitz(abs(rand()).^(0:p-1));
                     Ct(:,:,m*(s-1)+jf)=number*toeplitz(0.0.^(0:p-1));
            end
         end
%         beta=beta/2;
%         for jf=1:m
...
%                  Ct(:,:,m*(k-1)+jf)=number*toeplitz(0.0.^(0:p-1));
%         end
        %M=[M M];
        %M=zeros(p,k*m);
